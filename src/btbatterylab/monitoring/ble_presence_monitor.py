...
class BlePresenceMonitor:
    """
    Translates the JSONL events generated by BleMonitor into an
    ...
    def process_json_line(self, line: str) -> None:
        """
        Processes a single JSONL line and prints the resulting state.

        The printing lives here (not in JsonlTailMonitor) because only
        the consumer knows the shape of its own internal state.
        """
    ...
    def load_file(self, file_path: str | Path) -> DeviceStatus:
        """
        Loads and processes an entire JSONL file.
        """
